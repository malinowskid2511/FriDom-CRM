...
function validateAndSet(file: File) {
  ...
  if (file.type !== 'application/pdf') {
    error.value = 'Dozwolone są tylko pliki PDF'
    return
  }
  if (file.size > 10 * 1024 * 1024) {
    error.value = 'Plik jest za duży (maks. 10 MB)'
    return
  }
...
<template>
  <div>
    ...
    >
      ...
      <template v-else>
        <p class="text-sm font-medium">Przeciągnij plik PDF lub kliknij, aby wybrać</p>
        <p class="text-xs text-brand-black/60 mt-1">Maks. 10 MB</p>
      </template>
    </div>
    <button
      v-if="modelValue"
